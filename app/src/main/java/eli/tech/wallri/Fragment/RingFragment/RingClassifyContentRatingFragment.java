...
import eli.tech.wallri.Adapter.Rings.RingContentRattingRecyclerViewAdapter;
...
public class RingClassifyContentRatingFragment extends Fragment {
    ...
    private RingContentRattingRecyclerViewAdapter adapter;
    private int n = 20;
    private int s = 0;
    ...
    public View onCreateView(@NonNull LayoutInflater inflater, @Nullable ViewGroup container, @Nullable Bundle savedInstanceState) {
        ...
        OKhttpManager.getInstance().asyncJsonStringByURL(ratingUrl, new OKhttpManager.Func1() {
            ...
            public String onResponse(String result) {
                ...
                Gson gson = new Gson();
                RingNewsbean ringNewsbean = gson.fromJson(result, RingNewsbean.class);
                list = ringNewsbean.getRingtones();
                ...
                ((DefaultItemAnimator) mNewsRecyclerViewRing.getItemAnimator())
                        .setSupportsChangeAnimations(false);
                mNewsRecyclerViewRing.addItemDecoration(new DividerItemDecoration(getActivity(),
                        DividerItemDecoration.VERTICAL));
                adapter = new RingContentRattingRecyclerViewAdapter(mRingrAdsList, imageList, list, getActivity(),
                        flagList, mediaPlayer, isplayingList, downLoadFlag);
                mNewsRecyclerViewRing.setAdapter(adapter);

                setRingFeedNativeAds();
                nativeAd.loadAds();
                //条目点击事件
                adapter.setOnItemClickListener(new RingContentRattingRecyclerViewAdapter.onItemClickListener() {
                    @Override
                ...
                mNewsRefreshRing.setOnRefreshListener(new RefreshListenerAdapter() {
                    ...
                    public void onLoadMore(TwinklingRefreshLayout refreshLayout) {
                        ...
                        s = s + 20;
                        String mLoadMoreUrl
                                = "http://rington.co/android/picyap-rw/ringtones/list.php?v="
                                + category_name + "&loc=zh&s=" + s + "&n=" + n + "&sort=rating";
                        OKhttpManager.getInstance().asyncJsonStringByURL(mLoadMoreUrl,
                                new OKhttpManager.Func1() {
                                    @Override
                                    public String onResponse(String result) {
                                        int page = 0;
                                        Gson gson = new Gson();
    ...
    public void onMoonEvent(MessageEvent messageEvent) {
        ...
        try {
            ...
            if (messageEvent.getMessage() == 407) {
                EventBus.getDefault().post(new MessageEvent(401));
            ...
            } else if (messageEvent.getMessage() == 3330) {
                for (int i = 0; i < list.size(); i++) {
                    imageList.get(i).image = R.drawable.play;
                    imageList.get(i).max = 10000000;
                    imageList.get(i).progress = 0;
    ...
    private void initView(View itemView) {
        ...
        ratingUrl = "http://rington.co/android/picyap-rw/ringtones/list.php?v="
                + category_name + "&loc=zh&s=0&n=20&sort=rating";
    ...
    private void Refresh() {
        ...
        if (networkAvailable) {
            ...
            Toast.makeText(getActivity(),
                    "The current network is not available, please check the network connection!"
    ...
    private void setRingFeedNativeAds() {
        nativeAd = new NativeAdsManager(getActivity(), FacebookAdManager.RingCategoryRating_08, 5);
